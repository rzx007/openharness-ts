...
describe("resolveBootstrapRuntimeSelection", () => {
  ...
  it("keeps the saved provider/model pair when they still match", () => {
    expect(
      resolveBootstrapRuntimeSelection(models, "gemini-2.5-pro", "gemini"),
    ).toEqual({
      model: "gemini-2.5-pro",
      provider: "gemini",
    });
  ...
  it("derives the provider from the saved model when no provider is configured", () => {
    expect(
      resolveBootstrapRuntimeSelection(models, "gpt-5.4", undefined),
    ).toEqual({
      model: "gpt-5.4",
      provider: "openai",
    });
  ...
  it("falls back to the first available model when there is no saved runtime selection", () => {
    expect(
      resolveBootstrapRuntimeSelection(models, undefined, undefined),
    ).toEqual({
...
describe("DesktopSessionService context bridge", () => {
  it("forwards logical filters and entry ids without a storage path", async () => {
    const listContextEntries = vi.fn(async () => []);
    const removeContextEntry = vi.fn(async () => undefined);
    ...
    await service.removeContext({
      cwd: "D:/project",
      entryId: "package-manager",
    });

    expect(listContextEntries).toHaveBeenCalledWith({
      cwd: "D:/project",
      scope: "project",
      kind: "project_rule",
    });
    expect(removeContextEntry).toHaveBeenCalledWith("package-manager", {
      cwd: "D:/project",
    });
  });
});

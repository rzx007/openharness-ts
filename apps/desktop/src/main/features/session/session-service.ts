...
export class DesktopSessionService {
  ...
  private daemonStatus: DesktopDaemonStatus = createDaemonStatus(
    "idle",
    "等待连接 daemon",
  );
  private readonly subscriptions = new SessionSubscriptionRegistry();

  async listContext(input: DesktopContextQuery) {
    return (await this.getClient()).listContextEntries({
      cwd: requireString(input.cwd, "项目目录"),
      ...(input.scope ? { scope: input.scope } : {}),
      ...(input.kind ? { kind: input.kind } : {}),
    });
  }

  async addContext(input: DesktopContextEntryInput) {
    return (await this.getClient()).addContextEntry({
      cwd: requireString(input.cwd, "项目目录"),
      content: requireString(input.content, "记忆内容"),
    });
  }

  async updateContext(input: DesktopContextEntryUpdate) {
    return (await this.getClient()).updateContextEntry(
      requireString(input.entryId, "条目 ID"),
      {
        cwd: requireString(input.cwd, "项目目录"),
        content: requireString(input.content, "记忆内容"),
  ...
  async removeContext(input: DesktopContextEntryTarget): Promise<void> {
    await (
      await this.getClient()
    ).removeContextEntry(requireString(input.entryId, "条目 ID"), {
      cwd: requireString(input.cwd, "项目目录"),
    });
  }

  async listContextCandidates(input: { cwd: string }) {
    return (await this.getClient()).listContextCandidates({
      cwd: requireString(input.cwd, "项目目录"),
    });
  }

  async acceptContextCandidate(input: DesktopContextCandidateAccept) {
    return (await this.getClient()).acceptContextCandidate(
      requireString(input.entryId, "条目 ID"),
      {
        cwd: requireString(input.cwd, "项目目录"),
  ...
  async rejectContextCandidate(
    input: DesktopContextEntryTarget,
  ): Promise<void> {
    await (
      await this.getClient()
    ).rejectContextCandidate(requireString(input.entryId, "条目 ID"), {
      cwd: requireString(input.cwd, "项目目录"),
    });
  }

  async getContextPreview(input: { cwd: string }): Promise<string> {
    return (await this.getClient()).getContextPreview({
      cwd: requireString(input.cwd, "项目目录"),
    });
  }

  async getContextStatus(input: { cwd: string }) {
    return (await this.getClient()).getContextStatus({
      cwd: requireString(input.cwd, "项目目录"),
    });
  }
  ...
  async bootstrap(): Promise<DesktopBootstrapData> {
    ...
    const [settings, providers, allSessions, projectRecords, capabilities] =
      await Promise.all([
        client.getSettings(),
        client.listModels(),
  ...
  ): Promise<DesktopProjectDetails | null> {
  ...
  async createSession(
    ...
    const client = await this.getClient();
    const provider = await resolveProviderForModel(
      client,
      model,
      input.provider,
    );
    const projectId = input.projectId
      ? requireString(input.projectId, "Project ID")
      : undefined;
    const cwd = projectId
  ...
  async setProjectPinned(
    ...
    return await toDesktopProject(
      await (
        await this.getClient()
      ).setProjectPinned(input.projectId, input.pinned),
    );
  }
  ...
  async setProjectDefaultShell(
    ...
    return await toDesktopProject(
      await (
        await this.getClient()
      ).setProjectDefaultShell(input.projectId, input.shell),
    );
  }
  ...
  async removeProject(projectId: string): Promise<void> {
    await (
      await this.getClient()
    ).archiveProject(requireString(projectId, "Project ID"));
  }
  ...
  async resolveProjectDirectory(projectIdInput: string): Promise<string> {
    ...
    if (!info.isDirectory())
      throw new Error(`Project ${project.name} directory is unavailable.`);
    return project.path;
  }
  ...
  closeAuxSession(
    ...
    input: CloseDesktopAuxSessionInput,
  ): void {
    const subscriptionId = requireString(input.subscriptionId, "辅助订阅 ID");
    this.subscriptions.delete(
      webContentsId,
      auxiliarySubscriptionSlot(subscriptionId),
    );
  }
  ...
  async promoteQueuedPrompt(
    ...
    const expectedActiveRunId = requireString(
  ...
  async setDefaultModel(
    input: SetDefaultDesktopModelInput,
  ...
  async setSessionPinned(
    ...
    const client = await this.getClient();
    const session = (
      await client.listSessions({ includeArchived: true, limit: 1_000 })
  ...
  async dispose(): Promise<void> {
    ...
    try {
      ...
      if (registry?.pid === process.pid && registry.url === embeddedUrl)
        clearDaemonRegistry();
    } catch {
      clearDaemonRegistry();
    }
    ...
    try {
      while (!controller.signal.aborted && !webContents.isDestroyed()) {
        ...
        if (
          ...
          return;
        const view = toDesktopSessionView(
          update.value.state,
          sessionId,
          update.value.source,
        );
  ...
  private async connect(): Promise<OpenHarnessClient> {
    try {
      ...
      console.warn(
        "[session] registered daemon is unavailable, starting embedded daemon",
        error,
      );
      this.setDaemonStatus(
        "starting",
        "已注册 daemon 不可用，正在启动内置 daemon",
        {
    ...
    try {
      ...
      const { server, listen } = await startOpenHarnessDaemon({
        ...
        outsideProjectWorkspaceRoot: buildOutsideProjectRoot(
          app.getPath("documents"),
        ),
      });
      ...
      this.setDaemonStatus("error", "daemon 启动失败", {
        detail: errorMessage(error),
      });
      throw error;
    }
...
async function toDesktopProject(
...
async function verifyDaemonWithTimeout(
...
function readSettingsPermissionMode(
  ...
  if (
    ...
    return "default";
  return (
    normalizePermissionMode((permission as Record<string, unknown>)["mode"]) ??
    "default"
  );
}
...
function normalizePermissionMode(
  value: unknown,
): DesktopPermissionMode | undefined {
  return value === "default" || value === "plan" || value === "full_auto"
    ? value
    : undefined;
}
...
async function resolveProviderForModel(
  ...
  if (provider) {
    if (
      !models.some(
        (item) => item.id === model && item.providerName === provider,
      )
    ) {
  ...
  throw new Error(
    `模型 ${model} 在多个 provider 中同名，请明确指定 provider。`,
  );
}

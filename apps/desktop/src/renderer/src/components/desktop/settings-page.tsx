...
const personalNavigation: NavigationItem[] = [
  ...
  { label: "模型", icon: BrainCircuit },
  { label: "权限", icon: ShieldCheck },
  { label: "个性化", icon: Sparkles },
  { label: "键盘快捷键", icon: Keyboard },
  { label: "使用情况和计费", icon: CreditCard },
]

const integrationNavigation: NavigationItem[] = [
  { label: "插件", icon: Plug },
  { label: "MCP 服务", icon: Database },
  { label: "连接", icon: Link2 },
]

const codingNavigation: NavigationItem[] = [
  { label: "终端", icon: TerminalSquare },
  { label: "Git", icon: GitBranch },
  { label: "运行环境", icon: Code2 },
]
...
export function SettingsContent({ selectedSection }: SettingsContentProps): React.JSX.Element {
  return (
    <ScrollArea horizontal={false} className="h-full min-w-0 flex-1 bg-conversation">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-10 py-16 lg:px-16 lg:py-20">
        <header className="flex flex-col gap-2">
          <p className="text-xs font-medium tracking-wide text-muted-foreground">设置</p>
          <h1 className="font-heading text-3xl font-semibold tracking-tight">{selectedSection}</h1>

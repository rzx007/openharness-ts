...
describe("desktop session operation state", () => {
  ...
  it("reclaims older failed commands across targets while keeping other failures", () => {
    const runtime = {
      ...createEmptySessionRuntime(),
      operations: {
        "command-old": {
          id: "command-old",
          kind: "invoke-command" as const,
          phase: "failed" as const,
          sessionId: "session-1",
          target: "/old",
          startedAt: 1,
    ...
    expect(next.operations).not.toHaveProperty("command-old")
    expect(next.operations).not.toHaveProperty("command-other")
    expect(next.operations).toHaveProperty("edit-failed")
    expect(next.operations["command-new"]).toMatchObject({ phase: "pending", target: "/new" })
  })
})

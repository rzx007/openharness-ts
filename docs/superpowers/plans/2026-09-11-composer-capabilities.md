...
**架构：** `@` 与 `+` 共享一个 Context Picker；`/` 和 `$` 保持独立 Picker。所有选择结果使用结构化 composer item 传递，服务端负责解析、校验和执行；展示型系统状态使用 metadata presentation，不进入模型上下文。

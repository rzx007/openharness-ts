...
> 状态：当前最终方案。Goal 持久化、续跑、逐项完成审查、外部等待、运行时扩展边界、维护诊断和 Desktop 展示已实现；站点不在范围内，不提供旧目标数据迁移、双读或兼容层。
...
公开资料只用于确认 Goal 的产品语义和扩展思路。下面的数据结构、状态与调度规则是 OpenHarness 的最终设计，不宣称是 Codex 内部实现。
...
下图描述 OpenHarness 的最终设计，不代表已核实的 Codex 内部实现。Banner 由目标快照与事件持续更新；图中仅绘制首次展示入口。
...
flowchart TD
    ...
    Q -->|目标已变更或终结| DROP
    Q -->|逐项要求满足且证据有效| R["completed<br/>Banner 显示目标已完成"]
    Q -->|仍有明确下一步| S["事务检查续跑资格"]
    Q -->|需要用户回答或批准| T["waiting_user<br/>记录问题或批准请求 ID"]
    Q -->|连续三轮同一阻塞原因| U["blocked<br/>展示具体原因"]
    ...
    Q -->|有效外部句柄仍在运行| WAIT["保留 active<br/>按上限退避复查句柄"]
    WAIT -->|句柄完成| Q
    WAIT -->|句柄失败、失效或超时| V
...
Agent 每轮提交结构化评估建议：continue、complete、waiting_user 或 blocked，附 `progressAssessment`、进展、证据和下一步。`progressAssessment.kind` 为 `progress`、`waiting` 或 `no_progress`；`no_progress` 必须提供稳定的 `blockerKey`。服务端校验它对应当前 goal/revision/run，并决定状态或排队；没有有效评估时暂停并提示，不能盲目续跑。

完成评估必须逐项列出要求、来源、满足状态和证据引用，并列出剩余工作。完成证据应对应正文中的每项成功条件，例如测试报告、产物路径、实际查询结果。对于机器可检查的条件使用实际检查结果；无法自动判定的主观条件进入用户验收。模型自述“完成”或一条绿色测试不足以证明整个目标完成。

同一 `blockerKey` 连续三轮没有新产物、有效检查结果或能改变下一步的证据，进入 blocked；首次或原因变化只记录一次，不立即阻塞。运行中的外部任务有有效句柄时可等待其状态变化，并采用有上限的退避；不能通过重复相同总结将计数清零。新证据或用户修正条件才能解除该阻塞。

有效等待不增加也不清零 noProgressCount；由服务端保存句柄和截止时间，按 1、2、4、8、16、30 秒上限退避复查，不为每次轮询创建新 Agent 回合。纯等待轮询不消耗自动续跑次数，等待结束后实际启动的自动 Agent 回合消耗一次。每个外部等待必须记录截止时间；超时、句柄失效或跨会话句柄按失败处理并暂停。单个目标版本最多一个等待观察者，暂停或取消时必须清理观察者。
...
共享协议负责 Goal、进展评估和完成审查类型；SessionStore 负责持久状态、最近评估及唯一约束；会话应用服务负责创建/编辑/暂停/恢复；运行协调器负责持久 Run 和续跑；agent-runtime 通过可选的运行贡献提供目标提示和 `GoalAssessment`，不拥有目标状态机；core 只处理通用的本轮提示与本轮工具；Desktop 负责编辑和状态展示。Composer 仅提供入口。

Goal 状态和续跑必须留在宿主层。agent-runtime 没有 server/services 时仍可独立执行普通任务；未注入目标贡献时不出现目标提示或目标工具。目标运行通过宿主绑定的 goalId、revision 和 runId 上报事实，服务端负责证据来源、版本和状态结算。

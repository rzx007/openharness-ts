...
> 状态：当前实现。2026-09-12 已按 [Goal 实现复盘与调整方向](./session-goals-retrospective.md) 补强完成证据覆盖、进展判断、外部等待、扩展边界和维护诊断，并通过相关自动化验证；本轮 Desktop 手动交互复验仍待进行。站点不在范围内，不提供旧目标数据迁移、双读或兼容层。
...
以上是初次设计时的产品依据。后续开源代码调研已补充扩展接入、续跑与阻塞审查的依据，见 [2026-09-12 复盘](./session-goals-retrospective.md)。下面的数据结构、状态与调度规则均为 OpenHarness 的设计，不宣称是 Codex 内部实现。
...
实现完成后进行了第二轮代码审核。初版存在共享协议未导出、错误附件类型导入、非原子续跑和局部 UI 状态等问题；这些问题已通过共享解析器、持久 Run/请求事务、释放会话车道后结算、按 Run 绑定 GoalAssessment、按 scope 的目标 store 和真实启动验证修正。服务端保证完成证据来自当前目标版本、当前 Run 的成功工具结果；成功条件在业务语义上是否充分由 Agent 评估，缺少有效引用或需要主观判断时交由用户验收。

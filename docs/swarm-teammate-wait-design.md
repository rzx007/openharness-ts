...
- SwarmPanel 的 `duration` 在 teammate **运行期间不实时滚动**——只在 `created`/`completed`
  时 emit（无 `updated` 周期事件），所以运行中显示 ~0s，完成后跳到真实时长。要实时
  滚动需补一个周期性 `updated` emit，留待后续。

## 实现说明（与设计的偏差）

- `SwarmTeammateSnapshot.status` 的实际枚举是前端定义的 **`running | idle | done | error`**
  （不是本文上面写的 spawned/running/completed/failed）。实现以**前端枚举为准**：
  running/pending→running，completed+exit0→done，completed+非0 / failed / stopped→error。
- teammate 任务用 `type: "agent"` 标记（subprocess 后端的 TaskRunner 适配器注入），
  swarm_status 只对 `type==="agent"` 的任务发。

...
- **session checkpoint**（`session-memory/`）：每轮重写的**摘要**，给 compact
  连续性用，12k 封顶；
- **session 快照**（`sessions/<项目>-<哈希>/`）：完整消息历史 + 元数据，
  给 `--continue`/`--resume` 恢复用（latest/id 双写，load 侧配对修复）。
  `/dream` 扫的是后者。详见 [session-storage-design.md](./session-storage-design.md)。

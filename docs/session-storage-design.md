...
- 留待（审查后记录）：CLI 暂不传 toolMetadata（白名单实际未投喂，恢复只还原
  消息）；systemPrompt 传空串、usage 为 TS camelCase（与 Python 快照不互换）；
  exportSessionMarkdown 暂无 /export 命令消费；保存只在 exit/quit 触发，
  Ctrl+C 丢当次会话（latest 的"最新"语义打折）。

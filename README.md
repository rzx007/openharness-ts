...
- ✅ **Terminal** — daemon 统一持有本地 PTY，Desktop 右侧 Panel 与 Agent 连接同一个终端；支持多终端、输出快照恢复、REST/SSE 传输、对话卡片挂接，以及 `TerminalOpen/Send/Read/Signal/Close/List` 六个 Agent 工具。完整功能、权限和生命周期见 [docs/desktop-terminal-pty-design.md](docs/desktop-terminal-pty-design.md)。

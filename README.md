...
| `REPL`              | 交互式循环：`>` 提示符，约 40 个斜杠命令（`/help, /model, /provider, /clear, /compact, /permissions, /plan, /resume, /memory, /mcp, /skills, /agents, /subagents, /stats, /plugin, /reload-plugins, /output-style` 等）；支持输出样式 `default/minimal/codex`（`minimal` 极简纯文本渲染） |

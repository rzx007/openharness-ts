...
- ✅ **会话持久化** — TUI / 用户 print / 跨端主线使用 daemon `SessionStore`；单会话通过原子 snapshot + SSE 恢复。daemon 内 `Agent` 使用同一 store 持久化 child session；重启会保留审计记录并显式中断失去进程所有权的 run/workflow，不会伪造自动续跑。deprecated `--task-worker` compatibility fallback 仍用项目级快照。旧 print `--continue/--resume` 尚未迁到 daemon
...
- 🟢 **Daemon Session Runtime** — 主线具备 `ohs serve` / `ohs daemon start/status/stop`、Hono HTTP API、原子 session snapshot、SSE 事件流、单 session 串行/多 session 并发 run coordinator、持久化 PermissionBroker 和共享 `@openharness/client` reducer。默认 `ohs`（与 `ohs --tui`）启动/attach daemon；重启将遗留 run 和 daemon-owned workflow 明确收口，保留 parent/child session 与 timeline；进程内 REPL 入口已移除。详见 [docs/daemon-session-runtime-design.md](docs/daemon-session-runtime-design.md) 和 [docs/client-sync-flow.md](docs/client-sync-flow.md)
...
│  │   TUI / Print      │  │   OpenTUI + useServerSync            │  │

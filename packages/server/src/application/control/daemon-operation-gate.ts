...
export interface DaemonMaintenanceInfo {
  operationId: string;
...
export class DaemonOperationUnavailableError extends Error {
  constructor(
    readonly reason: "closing" | "blocked",
    readonly blocker?: DaemonMaintenanceInfo,
  ) {
    super(
      reason === "closing"
        ? "Daemon is closing"
        : blocker
          ? `Daemon operation is blocked by maintenance: ${blocker.operationName}`
          : "Daemon operation is blocked by maintenance",
    );
    this.name = "DaemonOperationUnavailableError";
  }
}

type BarrierRecord = DaemonOperationBarrier & {
  token: symbol;
  maintenance: DaemonMaintenanceInfo;
};
...
export class DaemonOperationGate {
  ...
  enter(scope: DaemonOperationScope): DaemonOperationLease {
    ...
    const blocker = [...this.barriers.values()].find((barrier) =>
      conflictsWithScope(barrier, scope),
    );
    if (blocker) throw new DaemonOperationUnavailableError("blocked", blocker.maintenance);
  ...
  tryEnterBarrier(
    barrier: DaemonOperationBarrier,
    isIdle: () => boolean,
    maintenance: DaemonMaintenanceInfo = {
  ...
  ): DaemonOperationLease | undefined {
    if (this.phase !== "open") return undefined;
    if ([...this.shared.values()].some((scope) => conflictsWithScope(barrier, scope)))
      return undefined;
    if ([...this.barriers.values()].some((active) => barriersConflict(barrier, active)))
      return undefined;
    if (!isIdle()) return undefined;
    const token = Symbol(`daemon-${barrier.kind}-barrier`);
    this.barriers.set(token, { token, ...barrier, maintenance });

...
export class AttachmentIntegrityService {
  ...
  async gc(input: { gracePeriodMs: number }): Promise<{
    ...
    try {
      const now = this.now();
      validateGracePeriod(input.gracePeriodMs);
      this.options.store.deleteExpiredAttachmentLeases(now);
      let deletedAssets = 0;
      let deletedBlobs = 0;
      let releasedBytes = 0;
      const assets = this.options.store.listAttachments({ includeDeleted: true });
      ...
      for (const asset of assets) {
        if (
          ...
          asset.deletedAt + input.gracePeriodMs > now ||
          !asset.sha256 ||
          !this.isCollectible(asset, activeLeases, now)
        ) {
          continue;
        }
      ...
      return { deletedAssets, deletedBlobs, releasedBytes };
    } finally {

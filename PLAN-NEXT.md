...
### 1. TUI 实际联调
- 启动 `oh --tui`，验证前端 spawn 后端、协议握手、ready 事件
- 测试消息发送/接收循环（user → submit_line → assistant_delta → assistant_complete）
- 测试权限弹窗（permission modal 的 y/n 交互）
- 测试工具调用的 tool_started / tool_completed 事件
- **验证命令**：`oh --tui "say hello"`
...
### 7. MCP Auth 流程
- 后端检测 MCP server 需要 auth 时发出 `modal_request { kind: "mcp_auth" }`
...
### 11. handleLine 命令路由
- 后端应识别 `/help`, `/model`, `/clear`, `/compact`, `/exit` 等斜杠命令
...
### 14. BackendHost 协议测试
- Mock QueryEngine 的 submitMessage 返回 StreamEvent 序列
- 验证 emit 的 BackendEvent 序列正确
- 验证 permission_request Future 超时处理
- 验证 shutdown 流程
...
| 优先级 | 任务 | 预估 |
|--------|------|------|
| P0 | TUI 联调 + BackendHost 测试 | 1-2 天 |
| P1 | Markdown + 主题 + resume | 2-3 天 |
| P2 | AskUser + MCP Auth + Swarm + 命令 | 3-5 天 |
| P3 | AppState + handleLine + 流式增强 | 2-3 天 |
| P4 | 组件测试 + 协议测试 + E2E | 3-5 天 |
